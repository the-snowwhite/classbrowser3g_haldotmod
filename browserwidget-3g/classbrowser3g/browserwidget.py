...
class ClassBrowser(object):
    ...
    def do_popup_menu(self, path, button, time):
        ...
        if filename is not None and line > 0:
            ...
            if fileext == '.hal' or fileext == '.dot':
                menuitem = Gtk.ImageMenuItem.new_from_stock('show-comp-dot', None)
                menu.append(menuitem)
                menuitem.show()
                def on_menuitem_show_dot(unused_menuitem):
                    print ("Right menu dot selected for file", filename[7:])
                    print ("Right menu dot selected for function", name)
                    #                file, fileext = os.path.splitext(filename)
                    runpath = os.path.join(os.path.dirname(__file__))
                    print("fileext = " +fileext)
                    if fileext == '.hal':
                        #                print("Run path = ", runpath)
                        net2dot = subprocess.Popen(["python", runpath + "/net2dot.py", filename[7:]],
                                               stdout=subprocess.PIPE,
                                               )

                        extractdot = subprocess.Popen(["python", runpath + "/extractdot.py", "-", name],
                                                  stdin=net2dot.stdout,
                                                  stdout=subprocess.PIPE,
                                                  )
                    elif fileext == '.dot':
                        extractdot = subprocess.Popen(["python", runpath + "/extractdot.py", filename[7:], name],
                                                  stdout=subprocess.PIPE,
                                                  )

                    showdot = subprocess.Popen(["python", runpath + "/xdot.py","-"],
                                           #shell=True,
                                           stdin=extractdot.stdout,
                                           # stdout=subprocess.PIPE,
                    )

                    output = showdot.stdout
                    for line in output:
                        print(line.decode('ascii').strip('\n'))
                menuitem.connect("activate", on_menuitem_show_dot)
