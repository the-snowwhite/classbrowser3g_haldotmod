...
class CTagsParser (ClassParserInterface):
    ...
    def _parse_doc_to_model(self, model):
        ...
        for tokens in result.splitlines():
            ...
            name_list = re.split(br'\W+', tokens[0])
            uri = Gio.File.new_for_path(tokens[1].decode('utf-8')).get_uri()
            lineno = int(tokens[2].rstrip(b';"'))
            typechar = b'' if len(tokens) <= 3 else tokens[3]
            ...
            for i in tokens[4:]:
                ...
                if key == b'access':
                    if value == b'private':
                        access = '_priv'
                    elif value == b'protected':
                        access = '_prot'
                elif key == b'file':
                    access = '_priv'
                else:
                    parent = re.split(br'\W+', value)
            ...
            while containers:
                ...
                if not parent:
                    ...
                    titer = model.append(None, parent[-1].decode('utf-8'), None, uri, lineno)
                    containers.append((parent, titer))
                    
            titer = model.append(titer, tokens[0].decode('utf-8'), typecode, uri, lineno)
            containers.append((parent + name_list, titer))
            
    _typecodes = {
            b'c': 'class', #class name
            b'd': 'define',
            b'e': 'enum', #enumerator
            b'f': 'function', #function or method name
            b'p': 'field',
            b'g': 'enum', #enumeration name
            b'm': 'field',
            b'v': 'field',
            b'n': 'namespace', #namespace
            b's': 'struct', #structure name
            b't': 'typedef',
            b'u': 'union', #union name
        }
